...
const TRINO_CATS = [
  'aggregate', 'array', 'binary', 'bitwise', 'color', 'comparison',
  'conditional', 'conversion', 'datasketches', 'datetime', 'decimal',
  'geospatial', 'hyperloglog', 'ipaddress', 'json', 'lambda',
  'map', 'math', 'regexp', 'row', 'session', 'setdigest', 'string',
  'system', 'table', 'teradata', 'tdigest', 'url', 'uuid', 'window',
];

function parseTrinoPage(html) {
  const fns = [];
  // Match dt with id + immediately following dd (multiline)
  const re = /<dt\s+class="sig sig-object py"[^>]*\bid="([^"]+)"[^>]*>([\s\S]*?)<\/dt>\s*<dd>([\s\S]*?)<\/dd>/gi;
  let m;
...
async function scrapeTrino() {
  ...
  for (const cat of TRINO_CATS) {
    ...
    try {
      ...
      let added = 0;
      for (const fn of fns) { if (!seen.has(fn.name)) { seen.set(fn.name, fn); added++; } }
...
const DUCKDB_CATS = [
  'numeric', 'char', 'date', 'time', 'timestamp', 'interval',
  'blob', 'list', 'array', 'struct', 'map', 'union',
  'aggregates', 'utility',
];
...
function parseDuckDBPage(html) {
  ...
  const re = /<h4[^>]*>[\s\S]*?<code[^>]*>([\s\S]*?)<\/code>[\s\S]*?<\/h4>([\s\S]*?)(?=<h4|<h3|<h2|$)/gi;
  let m;
  while ((m = re.exec(html)) !== null) {
