...
const lspWs = await page.evaluate(() => new Promise(resolve => {
...
if (editorCount > 0) {
  ...
  const sigHelp = await page.locator('.parameter-hints-widget').isVisible().catch(() => false);
  ...
  const suggestVisible = await page.locator('.suggest-widget').isVisible().catch(() => false);
  ...
  if (box) {
    ...
    const hoverText = (await page.locator('.monaco-hover-content').allTextContents().catch(() => [])).join(' ');
