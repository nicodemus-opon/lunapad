...
describe('OpenRouter live AI notebook editing', () => {
	maybeIt(
		...
		async () => {
			...
			throw new Error(
				`OpenRouter model did not emit apply_notebook_patch. Tools: ${allCalls
					.map((call) => call.tool)
					.join(', ')}`
			);
		},
	...
	maybeIt(
		...
		async () => {
			...
			throw new Error(
				`OpenRouter model did not emit a complex apply_notebook_patch. Tools: ${allCalls
					.map((call) => call.tool)
					.join(', ')}`
			);
		},
	...
	maybeIt(
		'can plan a deeply nested 5-node dashboard with tabs > columns > grid > card nesting',
		async () => {
			const requiredCells = [
				'revenue_by_month',
				'region_performance',
			...
			let messages: Array<{ role: 'user' | 'assistant'; content: string }> = [
				{
					role: 'user',
					content:
						'Edit the active notebook into a complex dashboard notebook named Growth Ops Deep Dive. Call apply_notebook_patch only. Do not create a separate notebook. Use JSON arrays as arrays, not strings. Add exactly five SQL executableCells with these short SQL bodies: ' +
						'revenue_by_month = SELECT 1 AS month, 100 AS revenue; ' +
						"region_performance = SELECT 'North' AS region, 100 AS revenue; " +
						"product_mix = SELECT 'Widget' AS product, 42 AS units; " +
						"customer_segments = SELECT 'Enterprise' AS segment, 10 AS customers; " +
						"churn_by_cohort = SELECT '2026-01' AS cohort, 0.05 AS churn_rate. " +
						'In blueprint.blocks build a nested layout that is at least 4 levels deep: a top-level tabs block, whose tab blocks contain a columns block, whose columns contain a grid block, whose grid contains a card block. ' +
						'Place a queryBlock for every one of the five cellIds somewhere inside that nesting (they do not all need to be at the deepest level, but each must be inside at least tabs > columns). ' +
						'Also add at least one metric widget referencing $revenue_by_month.revenue and one chart widget referencing $region_performance.'
				},
				{ role: 'assistant', content: '' }
			];
			const allCalls: Array<{ tool?: string; args?: Record<string, unknown> }> = [];
			...
			for (let turn = 0; turn < 6; turn++) {
				...
				if (patch) {
					...
					const flatBlocks = blocks.flatMap(nestedBlocks);
					const blockTypes = flatBlocks.map((block) => String(block.type ?? ''));
					const queryBlockCellIds = new Set(
						flatBlocks
							.filter((block) => block.type === 'queryBlock')
							.map((block) => String(block.cellId ?? block.outputName ?? ''))
					);
					const missingCells = requiredCells.filter(
						(name) => !serializedCells.includes(name)
					);
					const missingPlacements = requiredCells.filter(
						(name) => !queryBlockCellIds.has(name) && ![...queryBlockCellIds].some((id) => id.includes(name))
					);
					const hasDeepNesting =
						blockTypes.includes('tabs') &&
						blockTypes.includes('columns') &&
						blockTypes.includes('grid') &&
						blockTypes.includes('card');
					const hasWidgets =
						blockTypes.includes('metric') && blockTypes.includes('chart');
					const complete =
						executableCells.length >= 5 &&
					...
					messages = [
						...messages,
						{
							role: 'user',
							content:
								`Tool results:\n\napply_notebook_patch was incomplete. It had ${executableCells.length}/5 executable cell(s) ` +
								`(missing: ${missingCells.join(', ') || 'none'}), queryBlock placements missing for: ${missingPlacements.join(', ') || 'none'}, ` +
								`block types seen: ${[...new Set(blockTypes)].join(', ')} (need tabs, columns, grid, card, metric, chart).\n\n` +
								'Repair it now with a single apply_notebook_patch call. Keep SQL one-line and short. Use arrays as arrays, not JSON strings. ' +
								'Include all five executableCells, all five queryBlock placements nested at least tabs > columns, plus the metric and chart widgets. Do not call create_notebook.'
						},
						{ role: 'assistant', content: '' }
					];
					continue;
				...
				messages = [
					...
					{
						role: 'user',
						content: `Tool results:\n\n${toolResult}\n\nNow call apply_notebook_patch with all five executableCells and a tabs > columns > grid > card nested layout containing five queryBlock placements plus a metric and a chart widget. Do not stringify arrays. Do not call create_notebook.`
					},
					{ role: 'assistant', content: '' }
				];
			}
			...
			throw new Error(
				`OpenRouter model did not emit a complete deeply nested apply_notebook_patch. Tools: ${allCalls
					.map((call) => call.tool)
					.join(', ')}`
			);
		},
