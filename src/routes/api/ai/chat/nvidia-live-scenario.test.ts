...
function runSql(
	sql: string
): { ok: true; columns: string[]; rows: Record<string, unknown>[] } | { ok: false; error: string } {
	try {
...
function makeCell(
	partial: Pick<Cell, 'id' | 'outputName' | 'code' | 'language' | 'cellType'>
): Cell {
	return {
		...partial,
		connectionId: null,
...
function toolCalls(events: Array<Record<string, unknown>>) {
	return events
		...
		.map(
			(event) => event.call as { tool?: string; callId?: string; args?: Record<string, unknown> }
		);
}
...
class NotebookState {
	...
	runQueryNode(cellId: string): string {
		...
		if (!result.ok) {
			const sameErrorAsBefore =
				target.lastResult && !target.lastResult.ok && target.lastResult.error === result.error;
	...
	for (let turn = 0; turn < maxTurns; turn++) {
		...
		for (const call of calls) {
			if (call.tool === 'create_notebook' || call.tool === 'apply_notebook_patch') {
				...
				call.tool === 'sample_data' ||
				call.tool === 'query_data'
			) {
		...
		if (calls.length === 0) {
			...
			resultParts.push(
				'No tool calls detected. Continue building with tool calls, or emit <done> if finished.'
			);
		}
		if (process.env.DEBUG_SCENARIO === '1') {
			debugLog(
...
describe('NVIDIA live: full realistic notebook build', () => {
	maybeIt(
		...
		async () => {
			...
			expect(
				sawDone,
				`Model never signalled <done>. Tools called: ${allToolNames.join(', ')}`
			).toBe(true);
			...
			for (const name of requiredOutputs) {
				expect(
					presentOutputs,
					`Missing required cell "${name}". Present: ${presentOutputs.join(', ')}`
				).toContain(name);
			}
			...
			const fctResult = state.cells.get(
				[...state.cells.values()].find((c) => c.outputName === 'fct_orders')!.cellId
			)?.lastResult;
...
describe('NVIDIA live: harder adversarial notebook build', () => {
	maybeIt(
		...
		async () => {
			...
			expect(
				docJson.includes('tabs') && docJson.includes('columns'),
				'expected a tabs+columns nested layout'
			).toBe(true);
			expect(/"tagName":"(each|group)"/.test(docJson), 'expected an each/group loop block').toBe(
				true
			);
