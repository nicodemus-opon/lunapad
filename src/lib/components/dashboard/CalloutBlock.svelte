<script lang="ts">
	...
	import type { CalloutBlock } from '$lib/types/gui-pipeline';
...
<div class="group/block relative rounded-lg border-l-4 {styles.border} {styles.bg} overflow-hidden">
	...
	<div class="absolute top-1 right-1 flex items-center gap-0.5 opacity-0 group-hover/block:opacity-100 transition-opacity z-10">
		<button
			class="text-[10px] px-1.5 h-5 rounded border border-border/60 text-muted-foreground hover:text-foreground transition-colors bg-background"
	...
	{:else}
		...
		>
			...
			<div class="flex-1 min-w-0">
				...
				{:else}
					<span class="text-xs opacity-60">Click to add text…</span>
				{/if}
			</div>
		</button>
	{/if}
