	...
	{#if externalConnections.length > 0}
		<div class="mt-2 flex flex-wrap gap-1">
			{#each externalConnections as connection (connection.id)}
				<button
					class="inline-flex items-center gap-1 rounded border px-2 py-0.5 text-2xs hover:bg-accent"
			...
			<div class="grid grid-cols-2 gap-2">
				...
				<div>
					...
					<label class="mt-1 flex cursor-pointer items-center gap-1.5">
						...
						/>
						<span class="text-2xs text-muted-foreground">Remember password</span>
					</label>
				</div>
				<div>
					<label
