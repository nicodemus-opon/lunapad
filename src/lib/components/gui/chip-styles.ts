...
export const CHIP =
	'stage-chip group/pill inline-flex h-6 shrink-0 items-center overflow-hidden rounded border border-border/60 bg-background font-mono text-xs transition-colors duration-150';

/** Appended to CHIP when the referenced column is invalid. */
export const CHIP_INVALID = 'border-destructive/40 bg-destructive/10 text-destructive';

/** Expanded inline edit form. */
export const CHIP_EDITING =
	'stage-chip inline-flex shrink-0 items-center gap-1 rounded border border-ring/60 bg-background px-1.5 py-0.5 ring-2 ring-ring/15';

/** Clickable segment inside a chip (the main label button). */
export const CHIP_SECTION =
	'inline-flex h-full items-center px-2 transition-colors duration-150 hover:bg-muted/60';

/** Hover-revealed remove (X) button inside a chip. */
export const CHIP_X =
	'inline-flex h-full items-center px-1.5 text-muted-foreground/60 opacity-0 transition-[opacity,color] duration-150 group-hover/pill:opacity-100 hover:text-destructive';

/** "+ add" affordance (dashed, monochrome). */
export const CHIP_ADD =
	'inline-flex h-6 shrink-0 items-center gap-1 rounded border border-dashed border-border/70 px-2 font-mono text-xs text-muted-foreground/70 transition-colors duration-150 hover:border-muted-foreground/50 hover:bg-muted/30 hover:text-foreground';
...
export const CHIP_SELECT =
	'h-full cursor-pointer bg-transparent px-1 font-mono text-xs text-muted-foreground outline-none transition-colors duration-150 hover:text-foreground';
