...
export const SQL_REVIEW_TOOLS: AIChatToolName[] = ['get_cell_result', 'get_lineage', 'query_data', 'validate_result'];
...
export const SUBAGENT_TOOLS: Record<NonNullable<import('$lib/types/ai-chat.js').AIChatRequest['subagentType']>, AIChatToolName[]> = {
...
export function classifyComplexity(userText: string): TaskComplexity {
	...
	if (/\b(retention|cohort|funnel|churn|clv|ltv|rfm|attribution|mrr|arr|dau|mau|wau|nps|cac|arpu|conversion.?rate|customer.?journey|survival.?analysis|user.?lifetime)\b/.test(text)) return 'complex';
	...
	const multiCell = /\b([5-9]|\d{2,})\s+(models?|cells?|tables?|metrics?|dimensions?)\b/i.exec(text);
...
export function classifyIntent(userText: string): ChatIntent {
	...
	if (/\b(fix|debug|error|fail|break|wrong|incorrect|broken)\b/.test(text)
		&& /\b(cell|query|model|sql|result|output|it|this)\b/.test(text)) {
		return 'debug';
	}

	// Modification intents: update/rename/delete — stay in standard loop
	if (/\b(update|change|rename|delete|remove|refactor|improve|optimize|check|review)\b/.test(text)) {
		return 'standard';
	}
	...
	const analyticsPattern = /\b(retention|cohort|funnel|churn|clv|ltv|rfm|attribution|mrr|arr|dau|mau|nps|cac|arpu|conversion.?rate|customer.?journey)\b/;
	...
	const hasModelNoun = /\b(model|metric|dimension|fact|staging|mart|transform|pipeline|report|analysis|analytics|table|view|cell|query|notebook)\b/.test(text);
	const hasCreationVerb = /\b(create|build|make|write|generate|model|implement|develop|add|plot|visuali[sz]e|chart|graph|summari[sz]e|aggregate|compute|calculate)\b/.test(text);
	// Verbs that explicitly build something new (not just chart/visualize which are also visualization verbs)
	const hasStrongCreationVerb = /\b(create|build|make|write|generate|implement|develop)\b/.test(text);
	...
	if (/^(what|why|how|where|when|who|explain|describe|show me|tell me|can you|could you|is there|are there|do we have)\b/.test(text)) {
		if (/\banalyz|\bdistribut|\boutlier|\btrace\b|\bbreakdown\b|\bspread\b|\bstat\b|\bprofile\b|\bexplain\b/.test(text)) {
			return 'investigation';
		}
		return 'standard';
	}
...
export function parseReviewResult(rawText: string): ReviewResult | null {
	...
	try {
		...
		const parsed = JSON.parse(json) as Partial<ReviewResult & { scores: Record<string, number>; total: number }>;
		...
		const total = scores
			? parsed.total ?? (scores.correctness + scores.completeness + scores.performance + scores.convention)
			: undefined;
		return {
